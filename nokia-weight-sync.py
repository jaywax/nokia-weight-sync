...
if command == 'setup':
    ...
    if service == 'garmin':
        ...
        url = config.get('domogik','domogik_rest_url') + config.get('domogik','weight_id') + "?value={0}".format(weight)
        r = requests.get(url, verify=False)
        url = config.get('domogik','domogik_rest_url') + config.get('domogik','fat_id') + "?value={0}".format(percent_fat)
        r = requests.get(url, verify=False)
        url = config.get('domogik','domogik_rest_url') + config.get('domogik','muscle_id') + "?value={0}".format(muscle_mass)
        r = requests.get(url, verify=False)
        url = config.get('domogik','domogik_rest_url') + config.get('domogik','water_id') + "?value={0}".format(percent_hydration)
        r = requests.get(url, verify=False)
        url = config.get('domogik','domogik_rest_url') + config.get('domogik','bone_id') + "?value={0}".format(bone_mass)
        r = requests.get(url, verify=False)
